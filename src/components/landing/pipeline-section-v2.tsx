...
export const FLOW_MAPLE: ExampleFlow = {
  id: 'maple',
...
export const FLOW_FOUNDRY: ExampleFlow = {
  id: 'foundry',
...
export function PipelineSectionV2() {
  return (
    <>
      {/* Lead-in header sits above the two sticky flows so the section reads as one unit */}
      <header className="relative bg-[oklch(0.15_0.005_250)] px-6 pt-24 pb-10">
        <div className="mx-auto max-w-6xl">
          <span className="text-xs font-medium uppercase tracking-[0.18em] text-primary">
            How it works
          </span>
          <h2 className="mt-3 text-balance text-[clamp(2rem,4vw,3rem)] font-light leading-[1.1] tracking-[-0.02em] text-foreground">
            Two brands. Two banners. The same five steps.
          </h2>
          <p className="mt-4 max-w-2xl text-pretty text-base text-muted-foreground">
            Scroll through each example — URL, brand, brief, HTML, PNG. Every step is visible,
            editable, version-controlled. No black box.
          </p>
        </div>
      </header>

      <Flow example={FLOW_MAPLE} anchorId="pipeline-maple" />
      <Flow example={FLOW_FOUNDRY} anchorId="pipeline-foundry" />
    </>
  );
}

function Flow({ example, anchorId }: { example: ExampleFlow; anchorId: string }) {
  ...
  useMotionValueEvent(scrollYProgress, 'change', (v) => {
    const bucket = Math.min(STEP_TEMPLATE.length - 1, Math.floor(v * STEP_TEMPLATE.length));
    setActive((prev) => (prev === bucket ? prev : bucket));
  });

  if (reduceMotion) {
    return <StaticFallback example={example} anchorId={anchorId} />;
  }
    ...
    >
      <div className="sticky top-16 flex h-[calc(100vh-4rem)] flex-col overflow-hidden">
        <div className="mx-auto grid h-full w-full max-w-6xl grid-cols-[1fr_1.4fr] gap-12 px-6 pt-10 pb-12">
          {/* Left column — example header + vertical stepper */}
          <div className="flex flex-col">
            ...
            <h3 className="mt-3 text-balance text-[clamp(1.5rem,2.4vw,2rem)] font-light leading-[1.15] tracking-[-0.02em] text-foreground">
              {example.title}
            </h3>
            ...
            <ol className="relative mt-8 flex-1 space-y-2">
              <span
                aria-hidden
              ...
              <motion.span
                ...
                animate={{ height: `${((active + 0.5) / STEP_TEMPLATE.length) * 100}%` }}
                transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
              />
              {STEP_TEMPLATE.map((step, i) => (
                <StepRow
                  key={`${example.id}-${step.label}`}
                  step={step}
                  index={i}
                  isActive={i === active}
                  isPast={i < active}
          ...
          {/* Right column — big preview, content swaps per active step */}
            ...
            <div className="relative aspect-[5/4] w-full max-w-[640px] overflow-hidden rounded-2xl border border-white/10 bg-[oklch(0.18_0.005_250)] shadow-2xl shadow-black/50">
              <div className="flex items-center justify-between border-b border-white/5 px-4 py-2">
                ...
                <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
                  {STEP_TEMPLATE[active].label} · step {active + 1}/{STEP_TEMPLATE.length}
                </span>
                <span className="w-8" />
              </div>
              <div className="relative h-full w-full">
                <AnimatePresence mode="wait">
                  <motion.div
                    key={`${example.id}-${STEP_TEMPLATE[active].label}`}
                  ...
                  >
                    <BigPreview kind={STEP_TEMPLATE[active].preview} example={example} />
                  </motion.div>
                </AnimatePresence>
              </div>
            </div>
...
function BigPreview({ kind, example }: { kind: PreviewKind; example: ExampleFlow }) {
  switch (kind) {
    case 'url':
      return (
        <PreviewFrame>
...
function StaticFallback({ example, anchorId }: { example: ExampleFlow; anchorId: string }) {
  return (
    <section id={anchorId} className="relative bg-[oklch(0.15_0.005_250)] px-6 py-16">
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 max-w-2xl">
          <span className="text-[10px] font-medium uppercase tracking-[0.18em] text-primary">
            {example.exampleLabel}
          </span>
          <h3 className="mt-2 text-balance text-[clamp(1.5rem,2.4vw,2rem)] font-light leading-[1.15] tracking-[-0.02em] text-foreground">
            {example.title}
          </h3>
        </div>
