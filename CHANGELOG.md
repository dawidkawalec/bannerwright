...
- **Opcja A — pure-art Nano Banana + HTML text overlay.** Radical rewrite of the image-first pipeline. NB no longer renders headline/subhead/CTA inside the image — instead it paints a pure artistic composition (architectural line-art, synthwave waveform, painterly editorial, cinematic photograph, glassmorphic data-viz, riso collage) with intentional negative space reserved for typography. A new Gemini Flash call (`extractBriefStructure`) splits the free-form brief into `{ headline, subhead, cta }` preserving the original language and diacritics. A new Gemini Vision call (`detectTextZones`) locates 1-3 negative-space zones on the artwork with recommended text color, scrim type, and (for CTA) brand-aware button fill. The server then composes a BannerTree where `canvas.background` is the NB artwork and the children are real HTML text/button nodes placed inside the zones — so the editor opens with crisp HTML typography in brand fonts and Polish diacritics render pixel-perfect (no more "wdrożęnia"/"konsultącję" pixel typos). NB calls now receive multimodal reference images (workspace logo + 2 KB screenshots) pre-cropped to the target aspect via a new `prepareReferences` helper (without this NB inherits aspect from the references and ignores the prompt's dimension text). Fallback chain: NB call fails → text-only Gemini; zones detection or compose fails → legacy `ENCODE_DESIGN_SYSTEM` Vision-transcription path. Preset descriptions updated to "Architectural line-art + clean typography overlay", "Synthwave gradient mesh + heavy typography overlay", etc. Two new `LlmOperation` values: `extract_brief`, `detect_zones`. Affects: `src/lib/ai/style-presets.ts`, `src/lib/ai/gemini.ts`, `src/lib/ai/extract-brief-structure.ts` (NEW), `src/lib/ai/prompts/detect-text-zones.ts` (NEW), `src/lib/generation/compose-tree-from-zones.ts` (NEW), `src/lib/renderer/prepare-references.ts` (NEW), `src/lib/generation/run-tree.ts`.

### Added

- **Original AI design panel in the tree editor.** Image-first generations now persist the Nano Banana reference PNG path on the generations row (new `reference_image_path` column, migration 0003_known_vanisher.sql). The TreeEditorShell sidebar gains an `<ReferenceImagePanel>` below Versions — thumbnail + hover overlay + click-to-open modal with the full-size design and a download link. Panel hides when no reference image (Auto preset / legacy banners).
- **Per-banner total cost badge.** Generation page header shows `$0.0XXX` next to format + version count, aggregated from every llm_usage row tied to the generation. Nano Banana fires before the generations row exists, so a new `linkLlmUsageToGeneration(workspace_id, generation_id, sinceMs)` helper back-fills the null-generation rows on insert — `getGenerationCostUsd` returns the full sum (NB + Vision + edits).
- **Auto preset warning chip.** When the user selects the Auto style preset on `/generations/new`, an amber pill appears: "Skips Nano Banana — cheaper (~$0.02) and faster (~25 s) but plainer output. Pick another preset for design-quality results." Stops users from accidentally landing on the text-only fallback when they wanted the image-first pipeline.

### Changed

- **Nano Banana prompt respects the target format.** Previously `buildDesignPrompt` always asked for a 1:1 square (1080×1080); Story / Landscape / Portrait formats got a stretched square crop. Now the prompt receives the format, calls `dimensionsFor()` + the new `aspectLabel()` helper, and tells the model the exact dimensions + aspect descriptor ("9:16 vertical story", "1.91:1 landscape banner", etc.) with an explicit "composition MUST fit this exact aspect ratio" line. Verified: Story brief produces a 768×1376 (1.79 ratio) reference image, not a square crop.
- **Polish diacritics rendering hint.** Added a line to the Nano Banana prompt telling the model to render ą/ę/ć/ł/ó/ś/ż/ź/ń and other non-Latin glyphs precisely without base-letter substitution. Verified on a Polish brief — Vision encode picked up `SZYBKI HOSTING`, `Niezawodność`, `Sprawdź ofertę` correctly.

### Fixed

- **Recharts width(-1) warnings on dashboard sparklines.** Stats-card containers had `w-32` (128px) but `ResponsiveContainer` only received percentage sizes; recharts measured -1 during initial layout. Added `min-w-[128px] flex-shrink-0` + explicit `minWidth`/`minHeight` props so the suggested fix is also documented in the warning if it ever appears again.
