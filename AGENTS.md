...
**Domain:** bannerwright.com · **License:** MIT · **Status:** Fazy 0-4 live in prod end-to-end. **Generation = pure-art + HTML overlay (Opcja A):** Gemini 3.1 Flash splits the brief into `{headline, subhead, cta}` preserving language/diacritics → Nano Banana paints a pure-art composition (zero text rendered) at the target aspect with multimodal refs (logo + KB screens pre-cropped via `prepareReferences`) → Gemini 3 Pro Vision detects 1-3 negative-space zones on the artwork → server composes a BannerTree with the NB artwork as `canvas.background` and real HTML text/button nodes inside the zones (brand fonts, pixel-perfect Polish diacritics). Style presets: Auto (text-only fast draft, amber-warned) · Minimalist (architectural line-art) · Bold (synthwave mesh) · Editorial (painterly) · Photographic (cinematic) · Glassmorphic (data-viz panels) · Brutalist (riso collage). Each generation persists the NB reference PNG ("Original AI design" panel) + total-cost badge (extract_brief + image_gen + detect_zones + edits, back-linked via `linkLlmUsageToGeneration`). Fallback chain: NB fail → text-only Gemini; zones/compose fail → legacy `ENCODE_DESIGN_SYSTEM` Vision-transcription. Plus: auth, workspaces, onboarding hero, KB ingestion + brand auto-detect, tree editor (Layers/Canvas/Inspector + versions panel + tree-aware restore + mobile Sheet layout), AI chat edit, brand logo in generated banners, templates with parent_generation lineage, generated-assets library, PNG export, brand-preview swatches + variant shortcut on the form. Faza 5 infra ready (CI + GHCR release workflow + docker-compose.prod.yml + README quickstart); only v0.1.0 tag + repo transfer + demo materials pending
