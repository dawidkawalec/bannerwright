...
**Domain:** bannerwright.com · **License:** MIT · **Status:** Fazy 0-4 live in prod end-to-end. **Generation = image-first pipeline:** Nano Banana paints the complete banner (visual + text) at the target format's exact aspect ratio (square/story/landscape/portrait), Gemini 3 Pro Vision encodes it into an editable BannerTree (Mode A pure reconstruction). Style presets: Auto (text-only fast draft, amber-warned) · Minimalist · Bold · Editorial · Photographic · Glassmorphic · Brutalist. Each generation persists the Nano Banana reference PNG and shows it in the editor as "Original AI design" + a total-cost badge in the header (NB + Vision + edits, back-linked via linkLlmUsageToGeneration). Polish/non-Latin diacritics handled in the prompt. Plus: auth, workspaces, onboarding hero, KB ingestion + brand auto-detect, tree editor (Layers/Canvas/Inspector + versions panel + tree-aware restore + mobile Sheet layout), AI chat edit, brand logo in generated banners, templates with parent_generation lineage, generated-assets library, PNG export, brand-preview swatches + variant shortcut on the form, retry-on-empty validator. Faza 5 infra ready (CI + GHCR release workflow + docker-compose.prod.yml + README quickstart); only v0.1.0 tag + repo transfer + demo materials pending
