...
**Domain:** bannerwright.com · **License:** MIT · **Status:** Fazy 0-4 live in prod end-to-end. **Generation = image-first pipeline:** Nano Banana paints the complete banner (visual + text), Gemini 3 Pro Vision encodes it into an editable BannerTree (Mode A pure reconstruction — text nodes are real, editing stays clean). Style presets: Auto (text-only fast draft) · Minimalist · Bold · Editorial · Photographic · Glassmorphic · Brutalist. Plus: auth, workspaces, onboarding hero, KB ingestion + brand auto-detect, tree editor (Layers/Canvas/Inspector + versions panel + tree-aware restore + mobile Sheet layout), AI chat edit, brand logo in generated banners, templates with parent_generation lineage, generated-assets library, PNG export, brand-preview swatches + variant shortcut on the form, retry-on-empty validator on every tree generation. Faza 5 infra ready (CI + GHCR release workflow + docker-compose.prod.yml + README quickstart); only v0.1.0 tag + repo transfer + demo materials pending
